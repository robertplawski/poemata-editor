...
function App() {
  ...
  const { files, loading, error, deleteFile, createFile } = useFiles(
    setOpenFile,
    query,
  );

  const iframeSrc = useMemo(
    () => `${API_ROOT}/preview/${openFile}?template=${template}`,
    [openFile, template],
  );
      ...
      <div className="overflow-hidden flex flex-col  min-h-[100vh] max-h-[100vh] border-r-1 border-neutral-200 shadow-md">
        ...
        <div className="pl-4 pr-2 border-b-1 border-neutral-200 pb-2 flex flex-row items-center gap-2 w-full gap-4">
          ...
          <input
            ...
            className="border-1 border-neutral-200 rounded-xl flex-1 p-2 shadow-sm"
            value={query}
          ...
          {!loading &&
            ...
            files.map((file) => (
              <button
                ...
                key={file}
                className={
                  (openFile === file ? "font-bold " : "") +
                  "w-full -mx-2 p-2 border-b-1 border-neutral-200 flex flex-row gap-2 cursor-pointer"
                }
        ...
        <div className="border-t-1 justify-between  p-4 border-neutral-200  flex flex-row items-center gap-2 w-full gap-4">
          ...
          <div
            className={
              ...
              (openFile == null ? "opacity-[0.4] pointer-events-none" : " ")
            }
          >
            ...
            <IconButton
              onClick={openFile ? () => deleteFile(openFile) : () => { }}
            >
              <Trash />
            </IconButton>
          </div>
        ...
        <>
          {!loading && !error ? (
            <div className=" flex-1 flex border-r-1 border-neutral-200 flex-col justify-center overflow-y-hidden max-h-[100vh] items-center">
              ...
              <textarea
                ref={textAreaRef}
                className="outline-0 p-4 lg:p-8 w-full h-full resize-none whitespace-pre"
